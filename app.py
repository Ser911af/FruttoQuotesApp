...
df['Price'] = pd.to_numeric(df['Price'].astype(str)
                            ...
                            errors='coerce')
df = df.dropna(subset=['Price', 'cotization_date'])
...
# 3) Organic Status (depende de product+location)
...
# 4) Volume Unit (depende de lo anterior)
sub2 = sub.copy()
if organic != 'All':
...
# 5) Date range (depende de TODO lo anterior)
min_date = df['cotization_date'].dt.date.min()
max_date = df['cotization_date'].dt.date.max()
...
g = g[(g['cotization_date'].dt.date >= start_date) &
      (g['cotization_date'].dt.date <= end_date)]
...
if g.empty:
    ...
    # Preparar tabla con fecha y ordenada
    ...
    # Recomendación: vendor con el precio más bajo
