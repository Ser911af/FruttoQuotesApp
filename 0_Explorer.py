...
def fetch_all_quotations_from_supabase():
    """Trae quotations de Supabase; devuelve DF vacío ante error (sin romper UI)."""
    try:
        from supabase import create_client
    except Exception as e:
        st.error(f"Falta 'supabase' en requirements o import falló: {e}")
        return pd.DataFrame()
    ...
    try:
        ...
        st.error("No encontré SUPABASE_URL / SUPABASE_ANON_KEY en .streamlit/secrets.toml")
        return pd.DataFrame()
    ...
    try:
        ...
        st.error(f"No fue posible crear el cliente Supabase: {e}")
        return pd.DataFrame()
    ...
    # descartar filas sin precio válido
    ...
    # quitar posibles "PAS" en precio (texto de 'pasado' u otros)
...
with col1:
    st.subheader("FruttoFoods Quotation Tool")
with col2:
...
# Location (dependiente de producto)
